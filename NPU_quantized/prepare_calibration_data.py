...
OUTPUT_CALIB_DATA_DIR = "preprocessed_imagenet_calib_data" # Directory to save preprocessed numpy arrays
NUM_CALIB_SAMPLES = 100 # Number of images to use for calibration
IMAGE_SIZE = 224
...
def prepare_data():
    ...
    for img_path in tqdm(image_files[:NUM_CALIB_SAMPLES], desc="Preprocessing calibration images"):
        try:
            ...
            # Save as .npy file (some quantizers prefer raw binary or other formats)
            # For vai_q_onnx, a directory of .npy files, each containing one batch (of size 1 usually)
            # is a common way. The tool might also take raw .dat files.
            # Let's save as a single file for simplicity for now, and adapt if vai_q_onnx needs per-image files.
            # Vitis AI Quantizer for ONNX can take a directory of .pb or .npy files where each file is an input tensor.
